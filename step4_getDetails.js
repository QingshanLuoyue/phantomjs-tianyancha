...
while (!ins.atEnd()) { //循环读取文件内容
    ...
    linksArr.push(buffer.toString().split(',').pop());
    originXlsxData.push(buffer.toString().split(','));
    // console.log(buffer);
}
...
function startSearch(initialurl) {
    ...
    page.open(initialurl, function(status) {
        ...
        // console.log('搜索词语 = ', linksArr[searchCount]);
        if (status !== 'success') {
                    ...
                    if (content.address) {
                        ...
                        if (flag) {
                            ...
                            if (strArr.length >= 2) {
                                ...
                                var companyDiffArea = strArr[0].slice(0,5);
                            }
                        }
...
function writeToTxt(html) {
    ...
    var curData = originXlsxData[searchCount];
    var str = '';
